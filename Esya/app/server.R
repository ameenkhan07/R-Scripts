  ...
  output$map1 <- renderLeaflet({
    ...
    m <-data1 %>%
      ...
      addCircleMarkers(
        ...
        stroke = FALSE, 
        fillOpacity = 0.7)%>% 
        addLegend(pal = pal, values = c("Darkest Yellow", "Dark Yellow" , "Light Yellow" , "Yellow" , "No Colour"))
    m
  ...
  output$map2 <- renderLeaflet({
    ...
    m <- data2 %>%
      ...
      addCircleMarkers(
        ...
        stroke = FALSE, 
        fillOpacity = 0.7)%>% 
        addLegend(pal = pal, values = c("Darkest Red" , "Dark Red","Light Red","Red", "No Color"))
    m
  ...
  output$map3 <- renderLeaflet({
    ...
    m <- data3 %>%
      ...
      addCircleMarkers(
        ...
        stroke = FALSE, 
        fillOpacity = 0.7) %>% 
        addLegend(pal = pal, values = c("Darkest Green", "Dark Green", "No Colour"))
    m
  ...
  output$map4 <- renderLeaflet({
    ...
    m <- data4 %>%
      ...
      addCircleMarkers(
        ...
        stroke = FALSE, 
        fillOpacity = 0.7) %>% 
        addLegend(pal = pal, values = c("Darkest Blue","Dark Blue","No Colour"))
    m
